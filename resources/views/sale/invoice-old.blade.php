            ...
            <div class="centered">
                @if ($general_setting->site_logo)
                    <img src="{{ url('logo', $general_setting->site_logo) }}" height="42" width="50"
                        style="margin:10px 0;filter: brightness(0);width: 250px;">
                @endif
                ...
                <tbody>
                    ...
                    @foreach ($lims_product_sale_data as $key => $product_sale_data)
                        ...
                        <tr>
                            <td colspan="2">
                                ...
                                {{ number_format((float) ($product_sale_data->total / $product_sale_data->qty), 0, '.', '.') }}
                            ...
                            <td style="text-align:right;vertical-align:bottom">
                                {{ number_format((float) $product_sale_data->total, 0, '.', '.') }}</td>
                        </tr>
                    @endforeach
                    ...
                    <tr>
                        ...
                        <th style="text-align:right">
                            {{ number_format((float) $lims_sale_data->total_price, 0, '.', '.') }}</th>
                    </tr>
                    @if ($general_setting->invoice_format == 'gst' && $general_setting->state == 1)
                        <tr>
                            <td colspan="2">IGST</td>
                            <td style="text-align:right">{{ number_format((float) $total_product_tax, 0, '.', '.') }}
                            </td>
                        </tr>
                    @elseif($general_setting->invoice_format == 'gst' && $general_setting->state == 2)
                        <tr>
                            ...
                            <td style="text-align:right">
                                {{ number_format((float) ($total_product_tax / 2), 0, '.', '.') }}</td>
                        </tr>
                        <tr>
                    ...
                    @if ($lims_sale_data->order_tax)
                        <tr>
                            ...
                            <th style="text-align:right">
                                {{ number_format((float) $lims_sale_data->order_tax, 0, '.', '.') }}</th>
                        </tr>
                    @endif
                    @if ($lims_sale_data->order_discount)
                        <tr>
                            ...
                            <th style="text-align:right">
                                {{ number_format((float) $lims_sale_data->order_discount, 0, '.', '.') }}</th>
                        </tr>
                    @endif
                    @if ($lims_sale_data->coupon_discount)
                        <tr>
                            ...
                            <th style="text-align:right">
                                {{ number_format((float) $lims_sale_data->coupon_discount, 0, '.', '.') }}</th>
                        </tr>
                    @endif
                    @if ($lims_sale_data->shipping_cost)
                        <tr>
                            ...
                            <th style="text-align:right">
                                {{ number_format((float) $lims_sale_data->shipping_cost, 0, '.', '.') }}</th>
                        </tr>
                    @endif
                    <tr>
                        <th colspan="2" style="text-align:left">{{ trans('file.grand total') }}</th>
                        <th style="text-align:right">
                            {{ number_format((float) $lims_sale_data->grand_total, 0, '.', '.') }}</th>
                    </tr>
                    <tr>
                        @if ($general_setting->currency_position == 'prefix')
                            <th class="centered" colspan="3">{{ trans('file.In Words') }}:
                                {{--  <span>{{ $currency->code }}</span>  --}}
                                <span>{{ terbilang($lims_sale_data->grand_total) }}</span>
                            </th>
                        @else
                            <th class="centered" colspan="3">{{ trans('file.In Words') }}:
                                <span>{{ terbilang($lims_sale_data->grand_total) }}</span>
                ...
                <tbody>
                    @foreach ($lims_payment_data as $payment_data)
                        <tr style="background-color:#ddd;">
                            ...
                            <tr style="background-color:#ddd;">
                                <td style="padding: 5px;width:40%">Total:
                                    {{ number_format((float) $payment_data->amount, 0, '.', '.') }}</td>
                            </tr>
                            <tr style="background-color:#ddd;">
                                <td style="padding: 5px;width:40%">Tunai:
                                    {{ number_format((float) ($payment_data->amount + $payment_data->change), 0, '.', '.') }}</td>
                            </tr>
                            <tr style="background-color:#ddd;">
                                <td style="padding: 5px;width:30%">Kembali:
                                    {{ number_format((float) $payment_data->change, 0, '.', '.') }}</td>
                            </tr>
                    @endforeach
                    <tr>
