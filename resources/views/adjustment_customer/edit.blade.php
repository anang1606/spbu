...
@section('content')
    <section class="forms">
        <div class="container-fluid">
            {!! Form::open([
                'route' => ['adjustment-customer.update',$lims_adjustment_data->id],
                'method' => 'put',
                'files' => true,
                'id' => 'adjustment-form',
            ]) !!}
                                ...
                                <div class="col-md-12">
                                    <div class="table-responsive mt-3">
                                        <table id="myTable" class="table table-hover order-list">
                                            ...
                                            <tbody>
                                                @foreach ($lims_adjustment_data->details as $details)
                                                    <tr>
                                                        <td>{{ $details->product->name }}</td>
                                                        <td>{{ $details->group->name }}</td>
                                                        <td>{{ $details->product->code }}</td>
                                                        <td><input type="number" class="form-control qty" name="qty[]"
                                                                value="{{ $details->qty }}" required step="any" /></td>
                                                        <td class="action">
                                                            <select name="action[]" class="form-control act-val">
                                                                @if ($details->action == '+')
                                                                    <option value="+">{{ trans('file.Addition') }}
                                                                    </option>
                                                                    <option value="-">{{ trans('file.Subtraction') }}
                                                                    </option>
                                                                @else
                                                                    <option value="-">{{ trans('file.Subtraction') }}
                                                                    </option>
