...
class PurchaseController extends Controller
{
        ...
        if (!empty($purchases)) {
            foreach ($purchases as $key => $purchase) {
                ...
                if ($purchase->status == 1) {
                    ...
                    $nestedData['purchase_status'] = '<div class="badge badge-danger">Return</div>';
                    $purchase_status = 'Return';
                }

                if ($purchase->payment_status == 1)
                    $nestedData['payment_status'] = '<div class="badge badge-danger">' . trans('file.Due') . '</div>';
                elseif ($purchase->payment_status == 5)
                    $nestedData['payment_status'] = '<div class="badge badge-danger">Return</div>';
                else
                ...
                $nestedData['due'] = number_format($purchase->grand_total - $purchase->paid_amount, 0, '', '');
                ...
                $purchaseDate = date(config('date_format'), strtotime($purchase->created_at->toDateString()));
                $purchaseWarehouse = $purchase->warehouse;
                $purchaseNotes = preg_replace('/\s+/S', ' ', $purchase->note);

                $nestedData['purchase'] = array(
                    "[$purchaseDate,$purchase->reference_no,$purchase_status,$purchase->id,$purchaseWarehouse->name,$purchaseWarehouse->phone,$purchaseWarehouse->address,$supplier->name,$supplier->company_name,$supplier->email,$supplier->phone_number,$supplier->address,$supplier->city,$purchase->total_tax,$purchase->total_discount,$purchase->total_cost,$purchase->order_tax,$purchase->order_tax_rate,$purchase->order_discount,$purchase->shipping_cost,$purchase->grand_total,$purchase->paid_amount,$purchaseNotes,$user->name,$user->email]"
                );
                $data[] = $nestedData;
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data['shipping_cost'] = ($data['shipping_cost'] === '') ? 0 : (int)str_replace('.', '', $data['shipping_cost']);
            $data['order_discount'] = ($data['order_discount'] === '') ? 0 : (int)str_replace('.', '', $data['order_discount']);
            $data['paid_amount'] = ($data['grand_total'] === '') ? 0 : (int)str_replace('.', '', $data['grand_total']);
            ...
            $lims_account_list = Account::where('is_default', true)->first();

            $lims_payment_data = new Payment();
            $lims_payment_data->user_id = Auth::id();
            $lims_payment_data->purchase_id = $lims_purchase_data->id;
            $lims_payment_data->account_id = $lims_account_list->id;
            ...
            DB::commit();
            \Session::flash('message', 'Purchases created successfully');
            return redirect('purchases');
        } catch (\Exception $e) {
            DB::rollBack();
            \Session::flash('message', (config('app.debug')) ? $e->getMessage() : 'Error Exception in try action');
    ...
    public function update(Request $request, $id)
    {
        ...
        $lims_payment_data = Payment::where('purchase_id', $id)->first();
        $lims_payment_data->user_id = Auth::id();
        $lims_payment_data->amount = $data['grand_total'];
        $lims_payment_data->save();
    ...
    public function addPayment(Request $request)
    {
        ...
        $balance = (int)$lims_purchase_data->grand_total - $lims_purchase_data->paid_amount;
    ...
    public function updatePayment(Request $request)
    {
        ...
        $data['edit_amount'] = str_replace('.', '', $data['edit_amount']);
        $data['edit_paying_amount'] = str_replace('.', '', $data['edit_paying_amount']);
