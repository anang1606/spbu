...
class ReturnPurchaseController extends Controller
{
    ...
    public function returnData(Request $request)
    {
        ...
        if (!empty($returnss)) {
            foreach ($returnss as $key => $returns) {
                ...
                $nestedData['grand_total'] = $returns->grand_total;
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($lims_return_data) {
                $payment = Payment::where('purchase_id', $data['purchase_id'])->first();

                $lims_purchase_data->grand_total = $lims_purchase_data->grand_total - $data['total_cost'];
                $lims_purchase_data->paid_amount = $lims_purchase_data->paid_amount - $data['total_cost'];
                $lims_purchase_data->total_qty = $lims_purchase_data->total_qty - $data['total_qty'];
                $lims_purchase_data->total_cost = $lims_purchase_data->total_cost - $data['total_cost'];
                $lims_purchase_data->payment_status = 5;
                $lims_purchase_data->status = 5;

                $payment->amount = $payment->amount - $data['grand_total'];
            ...
            foreach ($product_id as $pro_id) {
                ...
                $lims_product_purchase = ProductPurchase::where([['product_id',$pro_id],['purchase_id',$data['purchase_id']]])->first();

                $lims_product_purchase->qty -= $qty[$key];
                $lims_product_purchase->total = $total[$key];
                $lims_product_purchase->save();
            ...
            DB::rollBack();
            // return $e->getMessage();
            return redirect('return-purchase')->with('message', 'Something Wrong');
        }
    }
