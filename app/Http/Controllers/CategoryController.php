    ...
    public function categoryData(Request $request)
    {
        ...
        if(!empty($categories))
        {
            foreach ($categories as $key=>$category)
            {
                ...
                if($category->image)
                    $nestedData['image'] = '<img src="'.url('images/category', $category->image).'">';
                else
                    $nestedData['image'] = '<img src="'.url('images/product/zummXD2dvAtI.png').'">';
                ...
                $nestedData['number_of_product'] = number_format($category->product()->where('is_active', true)->count(),0,'.','.');
                $nestedData['stock_qty'] = number_format($category->product()->where('is_active', true)->sum('qty'),0,'.','.');
                ...
                if(config('currency_position') == 'prefix')
                    $nestedData['stock_worth'] = 'Rp. '.number_format($total_price,0,'.','.').' / '.'Rp. '.number_format($total_cost,0,'.','.');
