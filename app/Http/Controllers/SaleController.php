...
class SaleController extends Controller
{
    ...
    {
        Notification::where([['type','App\Notifications\Penjualan'],['read_at',null]])
    ...
    public function saleData(Request $request)
    {
        ...
        if (empty($request->input('search.value'))) {
            ...
            $q = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')
                ...
                ->whereDate('sales.created_at', '=', date('Y-m-d', strtotime((int)str_replace('/', '-', $search))))
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir);
            if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                if ($sale->sale_status == 1) {
                    ...
                    $nestedData['sale_status'] = '<div class="badge badge-danger">Return</div>';
                    $sale_status = 'Return';
                }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data['paid_amount'] = ($data['paid_amount'] === '') ? 0 : (int)str_replace('.', '', $data['paid_amount']);
            $data['paying_amount'] = ($data['paying_amount'] === '') ? 0 : (int)str_replace('.', '', $data['paying_amount']);
            $data['grand_total'] = (int)str_replace('.', '', $data['grand_total']);
    ...
    public function addPayment(Request $request)
    {
        ...
        $data['paying_amount'] = (int)str_replace('.', '', $data['paying_amount']);
