...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $dash_return = $return;
            $dash_purchase_return = $purchase_return;
            $dash_profit = $profit;
        }
        else {
        ...
        // $start = strtotime(date('Y-m-01', strtotime('-12 month', strtotime(date('Y-m-d') ))));
        ...
        while($start < $end)
        {
            ...
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
                ...
                $sent_amount = DB::table('purchases')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->sum('grand_total');
                ...
                $sent_amount = DB::table('purchases')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
            ...
            $payment_recieved[] = number_format((float)$recieved_amount, 0, '', '');
            // $payment_purchase[] = number_format((float)$sent_amount, 0, '', '');
            $payment_purchase[] = number_format((float)$sent_amount, 0, '', '');
            $payment_sent[] = number_format((float)$expense_amount, 0, '', '');

            $yearly_laba_amount[] = number_format((float)$profit, 0, '', '');

            $month[] = $this->indo_month(date("F", strtotime($start_date)));
        ...
        DB::reconnect();
        return view('index', compact('revenue','payment_purchase', 'dash_purchase', 'dash_expense', 'dash_return',
        'dash_purchase_return',
        'dash_profit',
        'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount',
        'recent_sale','sale', 'recent_purchase', 'recent_quotation', 'recent_payment', 'best_selling_qty',
        'yearly_best_selling_qty', 'yearly_best_selling_price','yearly_laba_amount', 'all_permission'));
    }
