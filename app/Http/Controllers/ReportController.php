...
class ReportController extends Controller
{
    ...
    public function dailySale($year, $month)
    {
        ...
        if ($role->hasPermissionTo('daily-sale')) {
            ...
            while ($start <= $number_of_day) {
                ...
                $total_discount[$start] = number_format($sale_data[0]->total_discount, 0, '.', '.');
                $order_discount[$start] = number_format($sale_data[0]->order_discount, 0, '.', '.');
                $total_tax[$start] = number_format($sale_data[0]->total_tax, 0, '.', '.');
                $order_tax[$start] = number_format($sale_data[0]->order_tax, 0, '.', '.');
                $shipping_cost[$start] = number_format($sale_data[0]->shipping_cost, 0, '.', '.');
                $grand_total[$start] = number_format($sale_data[0]->grand_total, 0, '.', '.');
                $start++;
            }
    ...
    public function dailyPurchase($year, $month)
    {
        ...
        if ($role->hasPermissionTo('daily-purchase')) {
            ...
            while ($start <= $number_of_day) {
                ...
                $total_discount[$start] = number_format($purchase_data[0]->total_discount, 0, '.', '.');
                $order_discount[$start] = number_format($purchase_data[0]->order_discount, 0, '.', '.');
                $total_tax[$start] = number_format($purchase_data[0]->total_tax, 0, '.', '.');
                $order_tax[$start] = number_format($purchase_data[0]->order_tax, 0, '.', '.');
                $shipping_cost[$start] = number_format($purchase_data[0]->shipping_cost, 0, '.', '.');
                $grand_total[$start] = number_format($purchase_data[0]->grand_total, 0, '.', '.');
                $start++;
            }
            $start_day = date('w', strtotime($year . '-' . $month . '-01')) + 1;
            $prev_year = date('Y', strtotime('-1 month', strtotime($year . '-' . $month . '-01')));
    ...
    public function productReportData(Request $request)
    {
        ...
        if($request->input('search.value')) {
            ...
            $lims_product_all = Product::select('id', 'name', 'qty', 'is_variant','type')
                                ->where([
                                    ['name', 'LIKE', "%{$search}%"],
                                    ['is_active', true]
                                ])->offset($start)
                    ...
                    if(count($lims_product_return_data)) {
                        foreach ($lims_product_return_data as $product_return) {
                            if($product->type !== 'combo'){
                                ...
                                $returned_qty += $product_return->qty;
                            }
                        }
                    }
                    $nestedData['returned_qty'] = $returned_qty;
                    ...
                    if(count($lims_product_purchase_return_data)) {
                        foreach ($lims_product_purchase_return_data as $product_purchase_return) {
                            if($product->type !== 'combo'){
                                ...
                                $purchase_returned_qty += $product_purchase_return;
                            }
                        }
                    }
                    $nestedData['purchase_returned_qty'] = $purchase_returned_qty;
