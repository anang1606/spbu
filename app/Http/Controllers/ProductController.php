...
class ProductController extends Controller
{
    public function index()
    {
        ...
        if ($role->hasPermissionTo('products-index')) {
            ...
            $ingredients = false;
            return view('product.index', compact('all_permission', 'ingredients'));
        } else
    ...
    public function productData(Request $request)
    {
        ...
        if ($request->input('status') === '1') {
            $totalData = Product::where([['is_active', true], ['type', 'ingredients']])->count();
        } else {
            $totalData = Product::where([['is_active', true], ['type', '!=', 'ingredients']])->count();
        }
        ...
        if (empty($request->input('search.value'))) {
            if ($request->input('status') === '1') {
                $products = Product::with('unit')->offset($start)
                    ->where([['is_active', true], ['type', 'ingredients']])
                    ->limit($limit)
                    ->orderBy($order, $dir)
                    ->get();
            } else {
        ...
        if (!empty($products)) {
            foreach ($products as $key => $product) {
                ...
                $nestedData['qty'] = number_format($product->qty, 0, '.', '.');
                if ($product->unit_id)
                ...
                if (config('currency_position') == 'prefix')
                    $nestedData['stock_worth'] = 'Rp. ' . number_format(($product->qty * $product->price), 0, '.', '.') . ' / Rp. ' . number_format(($product->qty * $product->cost), 0, '.', '.');
                ...
                $nestedData['product'] = array(
                    '[ "' . $product->type . '"', ' "' . $product->name . '"', ' "' . $product->code . '"', ' "' .'"', ' "' . $nestedData['unit'] . '"', ' "' . $product->cost . '"', ' "' . $product->price . '"', ' "' . $tax . '"', ' "' . $tax_method . '"', ' "' . $product->alert_quantity . '"', ' "' . preg_replace('/\s+/S', " ", $product->product_details) . '"', ' "' . $product->id . '"', ' "' . $product->product_list . '"', ' "' . $product->variant_list . '"', ' "' . $product->qty_list . '"', ' "' . $product->price_list . '"', ' "' . $product->qty . '"', ' "' . $product->image . '"', ' "' . $product->is_variant . '"]'
                );
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')) {
            ...
            $ingredients = false;
            return view('product.create', compact('ingredients','lims_product_list_without_variant',
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data['cost'] = (int)str_replace('.', '', $data['cost']);
            $data['price'] = (int)str_replace('.', '', $data['price']);
            $data['is_active'] = true;
            $data['barcode_symbology'] = 'C128';
            $data['category_id'] = 1;

            if ($data['type'] == 'standard' || $data['type'] == 'ingredients') {
            ...
            if ($lims_product_data) {
                $data['product_id'] = $lims_product_data->id;
                $data['warehouse_id'] = 1;
                $product_warehouse = Product_Warehouse::create($data);
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            $ingredients = false;
            return view('product.edit', compact('ingredients','lims_product_list_without_variant',
            'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list',
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($lims_product_data->update($data)) {
                DB::commit();
                \Session::flash('create_message', 'Product created successfully');
            }
            return redirect(route('products.index'));
    ...
    {
        return Product::ActiveIngredients()->select('id', 'name', 'code')
