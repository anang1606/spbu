...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if((int)$request->role_id === 3){
            $nameRemoveSpace = str_replace(' ','',$data['name']);

            $checkEmail = User::where('name',$nameRemoveSpace)->get();
            if(count($checkEmail) > 1){
                $nameRemoveSpace .= count($checkEmail) +1;
            }

            $data['password'] = $data['pin'];
            $data['email'] = strtolower($nameRemoveSpace) . '@spbu.com';
        }
            ...
            if(!empty($request['pin'])){
                ...
                $this->validate($request, [
                    'old_pin' => [
                        ...
                        'string',
                        'min:6',
                        function ($attribute, $value, $fail) use($hashedPin) {
                            if (!Hash::check($value, $hashedPin)) {
                                return $fail('Pin lama tidak sesuai.');
                            }
                        },
                    ],
                ]);
