...
class ReturnController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($lims_return_data){
                ...
                $payment = Payment::where('sale_id',$data['sale_id'])->first();

                $sale->grand_total = $sale->grand_total - $data['total_price'];
                $sale->paid_amount = $sale->paid_amount - $data['total_price'];
                $sale->total_qty = $sale->total_qty - $data['total_qty'];
                $sale->total_price = $sale->total_price - $data['total_price'];
                $sale->payment_status = 5;
                $sale->sale_status = 4;
            ...
            foreach ($product_id as $pro_id) {
                ...
                $lims_product_sale = Product_Sale::where([['product_id',$pro_id],['sale_id',$data['sale_id']]])->first();

                $lims_product_sale->qty -= $qty[$key];
                $lims_product_sale->total = $total[$key];
                $lims_product_sale->save();
            ...
            DB::rollBack();
            // return $e->getMessage();
            return redirect('return-sale')->with('message', 'Something Wrong');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($lims_return_data){
            ...
            $sale->grand_total = $sale->grand_total - $data['grand_total'];
            $sale->paid_amount = $sale->paid_amount - $data['grand_total'];
            $sale->total_qty = $sale->total_qty - $data['total_qty'];
            $sale->total_price = $sale->total_price - $data['total_price'];
            $sale->payment_status = 5;
